...
public class PlayerControl : BaseRole
{
    ...
    private void UpdateControl()
    {
        ...
        // 测试
        if (true == InputHelper.GetButtonDown(KeyCode.K))
        {
            if (null != TimeEventManager.Instance)
            {
                System.Action fun = () =>
                {
                    Debug.Log("ExpireEvent CallBack !!");
                };

                TimeEventManager.Instance.AddTimeExpireEvent("Test", 3, fun);
            }
        }
    }
